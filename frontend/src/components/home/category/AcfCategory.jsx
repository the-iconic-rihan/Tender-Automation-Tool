...
import styles from "../../../assets/css/AcfCategory.module.css";
...
export default function AcfCategory() {
  ...
  const getCategoryData = async () => {
    ...
    await axios(config)
      .then(function (response) {
        ...
        if ("ACF" in data) {
          if ("acf_specifications" in data["ACF"]) {
            setCategoryData(data["ACF"]["acf_specifications"]["gpt_output"]);
            setSpecificationId(data["ACF"]["acf_specifications"]["_id"]);
          }
          setCategoryId(data["ACF"]["category_output_id"]);
        }
  ...
  return (
    <div className={styles.AcfCategory_Container}>
      <div className={styles.AcfCategory_container_heading}>
        <h2>ACF</h2>
        <button
      ...
      {showContentFlag && isLoading ? (
        <div className={styles.AcfCategory_container_content}>
          <div
            className={styles.AcfCategory_container_content_valves_container}
          >
            <div
              className={
                styles.AcfCategory_container_content_valves_container_1_row
              }
            >
              <h3
                className={
                  styles.AcfCategory_container_content_valves_container_heading
                }
              >
                Specification
              </h3>
              <button
                className={
                  styles.AcfCategory_container_content_valves_container_btn
                }
                onClick={() => {
            ...
            {showValvesFlag ? (
              <div
                className={
                  styles.AcfCategory_container_content_valves_container_2_row
                }
              >
                <p>{CategoryData}</p>
                <button
          ...
          <div
            className={styles.AcfCategory_container_content_downbtn_container}
          >
            <button
              className="category-download-btn"
              disabled={category.isLoading}
              onClick={() => {
                dispatch(downloadPDF({ fileId: categoryId, fileName: "ACF" }));

                // downloadPdf(categoryId, "ACF");
              }}
            >
              Download ACF
            </button>
